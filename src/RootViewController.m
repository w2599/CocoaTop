...
- (UIEdgeInsets)realUIContentInset {
  if (@available(iOS 11.0, *)) {
    ...
    // return UIEdgeInsetsMake(system.top + user.top, system.left + user.left,
    // system.bottom + user.bottom, system.right + user.right);
...
- (void)popupMenuTappedItem:(NSInteger)item {
  ...
  switch (item) {
  case 0:
    view = [[SetupViewController alloc] initWithStyle:UITableViewStyleGrouped];
    break;
  case 1:
    view =
        [[SetupColsViewController alloc] initWithStyle:UITableViewStyleGrouped];
    break;
  case 2:
    view = [[HtmlViewController alloc] initWithURL:@"guide"
                                             title:@"Quick Guide"];
    break;
  case 3:
    view = [[HtmlViewController alloc] initWithURL:@"story" title:@"The Story"];
    break;
  }
  if (view)
...
- (IBAction)hideShowNavBar:(UIGestureRecognizer *)gestureRecognizer {
  if (!gestureRecognizer ||
    ...
    // This "scrolls" tableview so that it doesn't actually move when the bars
    // disappear
    ...
    CGFloat slide =
        ...
        ([[NSUserDefaults standardUserDefaults] boolForKey:@"ShowHeader"]
...
- (void)viewDidLoad {
  ...
  [self
      popupMenuWithItems:@[ @"Settings", @"Columns", @"Quick Guide", @"About" ]
                selected:-1
                 aligned:UIControlContentHorizontalAlignmentLeft];
  ...
  UITapGestureRecognizer *twoTap = [[UITapGestureRecognizer alloc]
      initWithTarget:self
              action:@selector(hideShowNavBar:)];
  twoTap.numberOfTouchesRequired = 2;
  [self.tableView addGestureRecognizer:twoTap];

  filter = [[UISearchBar alloc]
  ...
  self.tableView.sectionHeaderHeight =
      self.tableView.sectionHeaderHeight * 3 / 2;
  ...
  [[NSUserDefaults standardUserDefaults] registerDefaults:@{
    ...
    @"ShortenPaths" : @YES,
    @"SortColumn" : @1,
    @"SortDescending" : @NO, // Main page (sort by pid)
    @"FilterColumn" : @0,
    @"ProcInfoMode" : @0,
    @"Mode0SortColumn" : @1001,
    @"Mode0SortDescending" : @NO, // Summary (by initial column order)
    @"Mode1SortColumn" : @2000,
    @"Mode1SortDescending" : @NO, // Threads (by thread id)
    @"Mode2SortColumn" : @3002,
    @"Mode2SortDescending" : @YES, // FDs (backwards by type)
    @"Mode3SortColumn" : @4001,
    @"Mode3SortDescending" : @NO, // Modules (by address)
  }];
  configChange = @"";
  configId = 0;
  selectedPid = -1;
...
- (void)searchBarTextDidBeginEditing:(UISearchBar *)searchBar {
  searchBar.placeholder =
      [NSString stringWithFormat:@"filter by %@ <tap another column>",
                                 filterColumn.fullname.lowercaseString];
}

- (void)searchBarTextDidEndEditing:(UISearchBar *)searchBar {
  searchBar.placeholder = [NSString
      stringWithFormat:@"filter by %@", filterColumn.fullname.lowercaseString];
}
...
- (void)preRefreshProcs:(NSTimer *)_timer {
  ...
  if (timer.isValid)
    [timer invalidate];
  timer = [NSTimer scheduledTimerWithTimeInterval:NTSTAT_PREQUERY_INTERVAL
                                           target:self
                                         selector:@selector(refreshProcs:)
                                         userInfo:nil
                                          repeats:NO];
}

- (void)refreshProcs:(NSTimer *)_timer {
  // Rearm the timer: this way the timer will wait for a full interval after
  // each 'fire'
  if (timerInterval >= 0.1 + NTSTAT_PREQUERY_INTERVAL) {
    if (timer.isValid)
      [timer invalidate];
    timer = [NSTimer scheduledTimerWithTimeInterval:(timerInterval -
                                                     NTSTAT_PREQUERY_INTERVAL)
                                             target:self
                                           selector:@selector(preRefreshProcs:)
                                           userInfo:nil
                                            repeats:NO];
  }
  ...
  // long)procs.threadCount, 			(float)procs.memFree / 1024 / 1024,
  ...
  if (@available(iOS 8, *)) {
    ...
    if (self.view.window == nil) {
      sizeClass = [UIApplication sharedApplication]
  ...
  if (shortLabel) {
    statusLabel.frame =
        CGRectMake(statusLabel.frame.origin.x, statusLabel.frame.origin.y,
                   self.tableView.frame.size.width - 80, 40);
    statusLabel.text =
        [NSString stringWithFormat:@"Free: %.1f MB  CPU: %.1f%%",
                                   (float)procs.memFree / 1024 / 1024,
                                   (float)procs.totalCpu / 10];
  } else {
    statusLabel.frame =
        CGRectMake(statusLabel.frame.origin.x, statusLabel.frame.origin.y,
                   self.tableView.frame.size.width - 150, 40);
    statusLabel.text = [NSString
        stringWithFormat:
            @"Processes: %lu   Threads: %lu   Free: %.1f/%.1f MB   CPU: %.1f%%",
            (unsigned long)procs.totalCount, (unsigned long)procs.threadCount,
            (float)procs.memFree / 1024 / 1024,
  ...
  if (_timer == nil) {
    ...
    if (idx != NSNotFound && procs[idx].display != ProcDisplayTerminated) {
      ...
      if (@available(iOS 7, *)) {
        ...
        [self.tableView deselectRowAtIndexPath:[NSIndexPath indexPathForRow:idx
                                                                  inSection:0]
                                      animated:YES];
      }
      // #endif
                 ...
                 boolForKey:@"AutoJumpNewProcess"]) {
    // If there's a new/terminated process, scroll to it
    NSUInteger idx = [procs indexOfDisplayed:ProcDisplayStarted];
    if (idx == NSNotFound)
    ...
    if (idx != NSNotFound) {
      ...
      if (last.display == ProcDisplayStarted ||
          last.display == ProcDisplayTerminated)
        idx = procs.count - 1;
      [self.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:idx
                                                                inSection:0]
                            atScrollPosition:UITableViewScrollPositionNone
                                    animated:YES];
    }
    // [self.tableView insertRowsAtIndexPaths:(NSArray *)indexPaths
    // withRowAnimation:UITableViewRowAnimationAutomatic] [self.tableView
    // deleteRowsAtIndexPaths:(NSArray *)indexPaths
...
- (void)sortHeader:(UIGestureRecognizer *)gestureRecognizer {
  ...
  for (PSColumn *col in columns) {
    ...
    if (filter.isFirstResponder && !filter.text.length) {
      ...
      filterColumn = col;
      [[NSUserDefaults standardUserDefaults] setInteger:col.tag
                                                 forKey:@"FilterColumn"];
      [self searchBarTextDidEndEditing:filter];
    } else {
      // Change sorting column
      sortDescending =
          sortColumn == col ? !sortDescending : col.style & ColumnStyleSortDesc;
      [header sortColumnOld:sortColumn New:col desc:sortDescending];
      sortColumn = col;
      [[NSUserDefaults standardUserDefaults] setInteger:col.tag
                                                 forKey:@"SortColumn"];
      [[NSUserDefaults standardUserDefaults] setBool:sortDescending
                                              forKey:@"SortDescending"];
      [timer fire];
    }
    break;
  }
...
- (void)scrollToBottom {
  [self.tableView
      scrollToRowAtIndexPath:[NSIndexPath
                                 indexPathForRow:[self.tableView
...
- (void)columnConfigChanged {
  ...
  configId++;
  columns =
      [PSColumn psGetShownColumnsWithWidth:UIApplication.sharedApplication
                                               .keyWindow.bounds.size.width];
  // Find sort column and create table header
  filterColumn =
      [PSColumn psColumnWithTag:[[NSUserDefaults standardUserDefaults]
                                    integerForKey:@"FilterColumn"]];
  [self searchBarTextDidEndEditing:filter];
  sortColumn = [PSColumn psColumnWithTag:[[NSUserDefaults standardUserDefaults]
                                             integerForKey:@"SortColumn"]];
  if (!sortColumn)
    sortColumn = columns[0];
  sortDescending =
      [[NSUserDefaults standardUserDefaults] boolForKey:@"SortDescending"];
  header = [GridHeaderView
      headerWithColumns:columns
                   size:CGSizeMake(self.tableView.bounds.size.width,
                                   self.tableView.sectionHeaderHeight)];
  footer = [GridHeaderView
      footerWithColumns:columns
                   size:CGSizeMake(self.tableView.bounds.size.width,
...
- (void)reappearAllView {
  ...
  NSString *configCheck =
      [NSString stringWithFormat:@"%d-%@", [def boolForKey:@"ShortenPaths"],
                                 [def stringForKey:@"FirstColumnStyle"]];
  ...
  // Hide filter bar
  CGFloat minOffset =
      filter.frame.size.height - self.tableView.realUIContentInset.top;
  if (self.tableView.contentOffset.y < minOffset)
    self.tableView.contentOffset = CGPointMake(0, minOffset);
  // Refresh interval
  timerInterval =
      [[NSUserDefaults standardUserDefaults] floatForKey:@"UpdateInterval"];
...
- (void)viewDidLayoutSubviews {
  ...
  if (self.view.window != nil) {
    if (@available(iOS 8, *)) {
      if (lastHorizationWindowSizeClass !=
        ...
        if (self.view.window.traitCollection.horizontalSizeClass ==
          ...
          statusLabel.frame =
              CGRectMake(statusLabel.frame.origin.x, statusLabel.frame.origin.y,
                         self.tableView.frame.size.width - 150, 40);
          statusLabel.text =
              [NSString stringWithFormat:@"Processes: %lu   Threads: %lu   "
                                         @"Free: %.1f/%.1f MB   CPU: %.1f%%",
...
- (UIView *)tableView:(UITableView *)tableView
  ...
  return [[NSUserDefaults standardUserDefaults] boolForKey:@"ShowHeader"] &&
                 !fullScreen
             ? header
             : nil;
}
...
- (UIView *)tableView:(UITableView *)tableView
  ...
  return [[NSUserDefaults standardUserDefaults] boolForKey:@"ShowFooter"] &&
                 !fullScreen
             ? footer
             : nil;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  GridTableCell *cell = [tableView
      dequeueReusableCellWithIdentifier:[GridTableCell
                                            reuseIdWithIcon:proc.icon != nil]];
  if (cell == nil)
    cell = [GridTableCell cellWithIcon:proc.icon != nil];
  [cell configureWithId:configId
                columns:columns
                   size:CGSizeMake(0, tableView.rowHeight)];
  if (proc)
  ...
  if (display == ProcDisplayTerminated) {
    if (@available(iOS 13, *)) {
      cell.backgroundColor =
          [UIColor colorWithDynamicProvider:^(UITraitCollection *collection) {
      ...
      cell.backgroundColor = [UIColor colorWithRed:1
                                             green:0.7
                                              blue:0.7
                                             alpha:1];
    }
    ...
    if (@available(iOS 13, *)) {
      ...
      cell.backgroundColor = [UIColor colorWithRed:0.7
                                             green:1
                                              blue:0.7
                                             alpha:1];
    }
  } else if (indexPath.row & 1) {
    // cell.backgroundColor = [UIColor colorWithRed:.95 green:.95 blue:.95
    // alpha:1];
    if (@available(iOS 13, *)) {
      cell.backgroundColor = UIColor.secondarySystemBackgroundColor;
    } else {
      cell.backgroundColor = [UIColor colorWithRed:.95
                                             green:.95
                                              blue:.95
                                             alpha:1];
    }
...
- (void)tableView:(UITableView *)tableView
              sendSignal:(int)sig
    toProcessAtIndexPath:(NSIndexPath *)indexPath {
  PSProc *proc = procs[indexPath.row];
...
- (void)tableView:(UITableView *)tableView
  ...
  [self.navigationController
      pushViewController:[[SockViewController alloc] initWithProc:proc]
                animated:anim];
}
